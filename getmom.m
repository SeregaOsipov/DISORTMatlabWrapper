function [PMOM] = getmom( IPHAS, GG, NMOM )
      HAZELM =[  2.41260, 3.23047, 3.37296, 3.23150, 2.89350, ...
      ...
      %remember that in fortran PMOM indexing starts from 0
%         Rayleigh phase function
      if ( IPHAS == 2 )
         PMOM(3) = 0.1;
      elseif ( IPHAS == 3 ) % Henyey-Greenstein phase fcn
         for K = 0:double(NMOM)
            PMOM(K+1) = GG^K;
         end
      elseif ( IPHAS == 4 ) % Haze-L phase function
        for K = 0:MIN(82,NMOM)
            PMOM(K+1) = HAZELM(K+1) / ( 2*K+1 );
        end
      elseif ( IPHAS == 5 )%Cloud C.1 phase function
        for K = 0:MIN(298,NMOM)
            PMOM(K+1) = CLDMOM(K+1) / ( 2*K+1 );
        end
    end
