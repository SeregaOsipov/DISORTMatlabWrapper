...
%%
% this one compiles DISORTMatlabWrapper (production)
% run this in the terminal
% cat DISOTESTAUX.f DISORT.f BDREF.f DISOBRDF.f ERRPACK.f LINPAK.f LAPACK.f RDI1MACH.f > code.f
% may have to add this flag to FFLAGS -fdefault-real-8
% Use -ffree-form flag to avoid the formatting hastle
% Use -fPIC flag for whatever reason
% DISORTMatlabWrapper.F90 is free form (F90) and the rest in code.f is fixed form (.f)
mex FFLAGS='-O3 -g -fcheck=all -fdump-core -fbounds-check -Wall -fPIC' /work/mm0062/b302074/workspace/fortran/DISORTMatlabWrapper/disort4.0.99/DISORTMatlabWrapper.F90 /work/mm0062/b302074/workspace/fortran/DISORTMatlabWrapper/disort4.0.99/code.f

% TODO: Make sure to choose correct BDREF.f (WILL CASE FATAL ERROR) or BDREFCopyFromTest.f
...
UMU0 = single(0.1); PHI0 = single(0.0);
...
FBEAM = single(pi / UMU0); FISOT = single(0.0);
...
TTEMP = single(0);
TEMIS = single(0);
display('MATLAB:before DISORTMatlabWrapper')
...
[RFLDIR, RFLDN, FLUP, DFDT, UAVG, UU, ALBMED, TRNMED] = DISORTMatlabWrapper(NLYR, DTAUC, SSALB, NMOM, PMOM, TEMPER, WVNMLO, ... %1-7
                    ...
                    PLANK, ONLYFL, ACCUR, PRNT, HEADER, ...%28-32
                    DELTAMPLUS, DO_PSEUDO_SPHERE)  % 33-34 , , ...  % EARTH_RADIUS
                    % H_LYR, RHOQ, RHOU, RHO_ACCURATE, BEMST, EMUST)%34-37
...
display('MATLAB:after DISORTMatlabWrapper')
...
SSALB( 1 ) = 1.0;
NMOM = int32(32);
PMOM = single(getmom( 3, 0.75, NMOM));
USRTAU    = true;
NTAU      = int32(2);
UTAU( 1 ) = 0.0;
USRANG    = true;
